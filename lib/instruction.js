...
function from(from) {
  ...
  // The dockerfile reference referes to the AS argument as "name"
  // We'll except `stage` to be kind
  const name = from.name || from.stage
...
/**
 * We'll take chown flags in a few ways. 
 * 1. An object with a `user` and/or `group` key.
 * 2. A `chown` property with a `user` and/or `group` key.
 * 
 * @returns [string] - May return an empty list.
 */
function getChownFlags(opts) {
  // Copy so we can mutate
  opts = Object.assign({}, opts)

  // special case - 0 is the only valid falsy value here
  if ( opts.chown === 0 ) opts.chown = '0'
  if ( opts.user === 0 ) opts.user = '0'
  if ( opts.group === 0 ) opts.group = '0'

  // any of these will produce some kind of --chown flag
  if ( !(opts.chown || opts.user || opts.group) ) {
    return null
  }

  let chown = opts.chown || { user: opts.user, group: opts.group };
  if ( chown.user || chown.group ) {
    // Its an object, force it to a string like user:group
    chown = `${chown.user || ''}:${chown.group || ''}`
    chown = chown.replace(/(^:)|(:$)/, '')
  } else {
    // Its a string or number so force it to string.
    chown = `${chown}`
  }
...
* Usage:
*   add({src: '', dest: ''}, true)
*   add({src: '', dest: '', onbuild: true})
*   add({src: '', dest: ''})
*   add({src: '', dest: '', user: '', group: ''})
* This version overrides previous user & group
*   add({src: '', dest: '', chown: 'user:group'})
* This version overrides previous chown
*   add({src: '', dest: '', chown: { user: '', group: ''}})
*
* Case (* preferred):
...
function add(add, onbuild) {
  ...
  let chown = getChownFlags(add);
  if (chown) {
    flags.push(chown);
  }

  // Copy this from the `add` object, makes easier use from serialized forms
  if ( onbuild == undefined && add.onbuild != undefined ) {
    onbuild = add.onbuild;
  }
...
function copy(copy, onbuild) {
  ...
  if (onbuild == undefined && copy.onbuild != undefined) {
